...
class DDPG(object):
    def __init__(self, state_dim, action_dim, max_action):
        self.memory = np.zeros((DDPG_CONFIG['MEMORY_CAPACITY'], state_dim * 2 + action_dim + 1),
                               dtype=np.float32)  # initialize memory初始化记忆库
        self.pointer = 0  # counter 计数器
    ...
    def learn(self):
        ...
        bt = torch.Tensor(self.memory[indices, :])  # 32存储记忆 打包
    ...
    def save(self):
        torch.save(self.actor.state_dict(), directory + 'actor.pth')  # save parameters
        torch.save(self.critic.state_dict(), directory + 'critic.pth')
        print("====================================")
        print("Model has been saved...")
        print("====================================")

    def restore(self):  # 提取参数 比直接load快
